...
def prepare_model_data(test_size=0.2, random_state=42):
    ...
    # Split the dataset
    X_train, x_test, Y_train, y_test = train_test_split(  # Removed unused variables
        features,
        targets,
        test_size=test_size,
...
def train_model(X_train, Y_train, model_type, n_estimators=100, random_state=42):
    """
    Trains the model
    Args:
    ...
    """
    # TODO the way model is selected should be improved
    # Instantiate machine learning model
    if model_type == MLModels.available_models[0]:  # Linear regression
        model = LinearRegression()

    elif model_type == MLModels.available_models[1]:  # Random forest
        model = RandomForestRegressor(n_estimators=n_estimators, random_state=random_state)

    elif model_type == MLModels.available_models[2]:  # Support Vector Machine
        model = svm.SVR()

    else:  # Invalid model passed in
        print("ERROR! Invalid model passed in. Exiting...")
        sys.exit(-1)
